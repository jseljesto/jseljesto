...
public class CreateTeam {
  ...
  /**
   * Screen to input name and nation of new club.
   * 
  ...
  public void findClubAndNation(Stage stage) throws FileNotFoundException, IOException {
    ...
    // Loading in nations
    // ArrayList<Nasjon> nasjonsListe = new ArrayList<Nasjon>(); //alle desse mao
    // modererast
  ...
  /**
   * Creates a new team and club.
   * @param nasjonsListe List of possible nations.
   * @param valgtNasjon The chosen nation.
   * @param clubName The inputed club name.
   * @param stage The current stage.
   * @throws FileNotFoundException
   * @throws IOException
   */
  public void createNewTeam(ArrayList<Nasjon> nasjonsListe, String valgtNasjon, String clubName, Stage stage)
    ...
    for (int k = 0; k < 20; k++) {
      ...
      /*
       * if ((importedTeam.get(k).firstName + importedTeam.get(k).surName).length() <=
       * 9) { System.out.println(importedTeam.get(k).firstName + " " +
       * importedTeam.get(k).surName + "\t" + "\t" + "\t" + "\t" +
       * importedTeam.get(k).nation + "\t" + "\t" + importedTeam.get(k).age + "\t" +
       * importedTeam.get(k).position + "\t" + importedTeam.get(k).average + "\t" +
       * importedTeam.get(k).lonn + "\t" + "�" + importedTeam.get(k).verdi); } if
       * ((importedTeam.get(k).firstName + importedTeam.get(k).surName).length() > 9
       * && (importedTeam.get(k).firstName + importedTeam.get(k).surName).length() <
       * 20) { System.out.println(importedTeam.get(k).firstName + " " +
       * importedTeam.get(k).surName + "\t" + "\t" + "\t" + importedTeam.get(k).nation
       * + "\t" + "\t" + importedTeam.get(k).age + "\t" + importedTeam.get(k).position
       * + "\t" + importedTeam.get(k).average + "\t" + importedTeam.get(k).lonn + "\t"
       * + "�" + importedTeam.get(k).verdi); } if ((importedTeam.get(k).firstName +
       * importedTeam.get(k).surName).length() >= 20) {
       * System.out.println(importedTeam.get(k).firstName + " " +
       * importedTeam.get(k).surName + "\t" + "\t" + importedTeam.get(k).nation + "\t"
       * + "\t" + importedTeam.get(k).age + "\t" + importedTeam.get(k).position + "\t"
       * + importedTeam.get(k).average + "\t" + importedTeam.get(k).lonn + "\t" + "�"
       * + importedTeam.get(k).verdi); }
       */
      writeTeam.println(importedTeam.get(k).kitNumber + " " + importedTeam.get(k).firstName + " "
          + importedTeam.get(k).surName + " " + importedTeam.get(k).nation + " " + importedTeam.get(k).age + " "
          + importedTeam.get(k).position + " " + importedTeam.get(k).average + " " + importedTeam.get(k).potential + " "
          + importedTeam.get(k).lonn + " " + importedTeam.get(k).verdi);
    }

    writeTeam.close();
    /*
     * System.out.println(""); System.out.println("Team Average: " +
     * teamAverage/20);
     */

    newClub.clubAvg = teamAverage / 20;
    writeClub.println(newClub.clubName + " " + newClub.clubAvg + " " + newClub.kamper + " " + newClub.seier + " "
        + newClub.uavgjort + " " + newClub.tap + " " + newClub.maol + " " + newClub.sleptInn + " " + newClub.maolFor
        + " " + newClub.poeng + " " + newClub.budsjett + " " + newClub.league);
